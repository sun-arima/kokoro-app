...
export default function FamilySetupSimplePage() {
  ...
  const updateSetting = (key: keyof typeof settings, value: boolean) => {
    if (key === 'shareRecord' && !value) {
      setSettings({ shareRecord: false, shareScore: false });
    } else {
      setSettings((prev) => ({ ...prev, [key]: value }));
    }
  };

  const allOff = !settings.shareRecord && !settings.shareScore;
          ...
          <Card className="bg-blue-50/50 border border-blue-200/50">
            {allOff ? (
              ...
              <>
                ...
                {settings.shareRecord && !settings.shareScore && (
